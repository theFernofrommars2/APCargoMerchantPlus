...
public class UnloadTask extends CargoTask
{
    ...
    public void execute(){
        ...
        originalPilot.sendMessage(CargoMain.SUCCESS_TAG + "Unloaded " + count + " worth $" + String.format("%.2f", count*item.getTradePrice()) + " took a tax of " + String.format("%.2f",CargoMain.getUnloadTax()*count*item.getTradePrice()));
        CargoMain.getEconomy().depositPlayer(originalPilot,count*item.getTradePrice()*(1-CargoMain.getUnloadTax()));
